    ...
    public int retrieve(String path) {
        ...
        for (int i = 0; i < path.length(); i++) {
            ...
            if (character == 'l' || character == 'L') {
                if (cur.left == null) {
                    throw new IllegalStateException("Invalid: left is null at step " + i);
                }
                cur = cur.left;
            } else if (character == 'r' || character == 'R') {
                if (cur.right == null) {
                    throw new IllegalStateException("Invalid: right is null at step " + i);
                }
                cur = cur.right;
            } else {
                throw new IllegalArgumentException("Path should contain only 'l' or 'r': got '" + character + "'");
            }
        }
        return cur.getRoot();
    }
